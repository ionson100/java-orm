...
//    WHERE relname = 'имя_вашей_таблицы';
...
//    }
    @Test
    void getTableName(){
        String name=Configure.getSession("pg").getTableName(MyTable.class);
        assert name!=null;
 ...
 @Test
    ...
    void update() throws Exception {
        ...
        try (ISession session = Configure.getSession("pg")) {
            ...
            session.insertBulk(myTable);
            list = session.getList(MyTable.class, null);
            list.getFirst().name = "1234";
            session.update(list.getFirst());
            list = session.getList(MyTable.class, null);
        }
        assert list.getFirst().name.equals("1234");
    ...
    void autoClose() throws Exception {
        ...
        Configure.getSessionAuto("pg").dropTableIfExists(MyTable.class);
        Configure.getSessionAuto("pg").createTableIfNotExists(MyTable.class);
        boolean alive=Configure.getSessionAuto("pg").IsAlive();
        if(alive==true){
            throw new Exception();
        }
        ...
        for (int i = 0; i < 10; i++) {
            ...
            myTable.name = "123";
            myTable.stringList.add("123");
            Configure.getSessionAuto("pg").insertBulk(myTable);
        }
    }
    ...
    void transaction() throws Exception {
        ...
        try (ISession session = Configure.getSession("pg")) {
            ...
            List<MyTable> list=Configure.getSessionAuto("pg").getList(MyTable.class,null);
            assert  list.size()==0;
    ...
    void transaction2() throws Exception {
        ...
        try (ISession session = Configure.getSession("pg")) {
            ...
            assert  list.size()==0;
            session.commitTransaction();
            list=Configure.getSessionAuto("pg").getList(MyTable.class,null);
            assert  list.size()==1;
    ...
    void appender(){
        ...
        try (ISession session = Configure.getSession("pg")) {
            session.dropTableIfExists(MyAppender.class);
            session.createTable(MyAppender.class);

            MyAppender myTable = new MyAppender();
            myTable.name = "123";
            myTable.integerList.add(123);
            myTable.stringList.add("ion100");

            session.insert(myTable);

           List<MyAppender> list=Configure.getSessionAuto("pg").getList(MyAppender.class,null);
            assert  list.size()==1;
