...
public class TestFluentMySql {
    ...
    void testAsync() throws Exception {
        ...
        Configure.getSessionAuto("my").query(MyTable.class).toListAsync().thenAccept(myTables -> {
    ...
    public void TestSingle() throws Exception {
        ...
        try (ISession session = Configure.getSession("my")) {
            ...
            Thread.sleep(100);
            assert (l.size() == 1);
            var t=session.query(MyTable.class).getByPrimaryKey(2);
            assert t!=null;
        }
    ...
    private static void extracted(ISession session) throws Exception {
        session.query(MyTable.class).dropTableIfExists();
        session.query(MyTable.class).createTableIfNotExists();
        session.execSQL("CREATE FULLTEXT INDEX  t22_name ON `t22` (name(20)); ");
        for (int i = 0; i < 10; i++) {
            MyTable myTable = new MyTable();
            myTable.age = i;
            myTable.name = "name" + i;
