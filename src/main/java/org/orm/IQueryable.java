...
public interface IQueryable<T> {
     ...
     * </pre>
     */
    IQueryable<T> where(String where, Object... parameters);
     ...
     * </pre>
     */
    IQueryable<T> orderBy(String columnName);

    /**
     * Adds the ORDER BY desk parameter to the database query.
     *
     * @param columnName the column name
     * @return the queryable
     *
     ...
     * {@code
     * ISession session=Configure.getSession();
     * var list = session.query(MyTable.class).where(" age=?",30).orderByDesk("name").orderBy("age").toList();
     * }
     * </pre>
     */
    IQueryable<T> orderByDesk(String columnName);
     ...
     * </pre>
     */
    IQueryable<T> update(String columnName, Object value);
     ...
     * @return IQueryable
     */
    IQueryable<T> rawSqlSelect(String sql);
    ...
    Map<Object, List<T>> groupBy(String columnName);
     ...
     * </pre>
     */
    List<Object> distinctBy(String columnName);
     ...
     * </pre>
     */
    List<?> select(String columnName);


    /**
     * Getting a list of values for one field of a database table
     * @param expression The expression to select from the database
     * @return list result
     */
    List<?> selectExpression(String expression);
     ...
     * </pre>
     */
    CompletableFuture<List<?>> selectAsync(String columnName);
     ...
     * </pre>
     */
    CompletableFuture<List<?>> selectExpressionAsync(String expression);
     ...
     * </pre>
     */
    CompletableFuture<Map<Object, List<T>>> groupByAsync(String columnName);
     ...
     * </pre>
     */
    CompletableFuture<List<Object>> distinctByAsync(String columnName);
     ...
     * </pre>
     */
    CompletableFuture<Object> countAsync();
    ...
    /** Gets a wrapper around the CompletableFuture's dropTableIfExists method, allowing non-blocking operations to be performed on the main thread.
     * @return CompletableFuture &lt;Void&gt;
     */
    CompletableFuture<Void> dropTableIfExistsAsync();

    /** Gets a wrapper around the CompletableFuture's createTableIfNotExistsAsync method, allowing non-blocking operations to be performed on the main thread.
     * @return  CompletableFuture &lt;Void&gt;
     */
    CompletableFuture<Void> createTableIfNotExistsAsync();

    /** Gets a wrapper around the CompletableFuture's createTableAsync method, allowing non-blocking operations to be performed on the main thread.
     * @return CompletableFuture &lt;Void&gt;
     */
    CompletableFuture<Void> createTableAsync();

    /**
     * Gets a wrapper around the CompletableFuture's iterator method, allowing non-blocking operations to be performed on the main thread.
     * @param task Called on each cursor iteration
     * @return CompletableFuture &lt;Void&gt;
     */
    CompletableFuture<Void> iteratorAsync(ITask<T> task);

    /**
     * Get an Object from a Database by Primary Key
     * @param primaryKey Primary key value
     * @return  object TYPE OF the class marked with @{@link MapTable} or @{@link MapTableName}
     */
    T getByPrimaryKey(Object primaryKey);

    /**
     * Gets a wrapper around the CompletableFuture's getByPrimaryKey method, allowing non-blocking operations to be performed on the main thread.
     *
     * @param primaryKey Primary key value
     * @return T object
     *
     ...
     * </pre>
     */
    CompletableFuture<T> getByPrimaryKeyAsync(Object primaryKey);
