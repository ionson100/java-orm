...
class ScopedValue<T> implements IQueryable<T> {
    ...
    public IQueryable<T> where(String where, Object... parameters) {
    ...
    @Override
    public IQueryable<T> limitOffSet(int rowCount, int offSet) {
        this.limit = " LIMIT " + rowCount + " OFFSET " + offSet + " ";
        return this;
    }

    public IQueryable<T> orderBy(String columnName) {
    ...
    @Override
    public IQueryable<T> orderByDesk(String columnName) {
    ...
    @Override
    public IQueryable<T> update(String columnName, Object value) {
    ...
    @Override
    public IQueryable<T> rawSqlSelect(String sql) {
    ...
    public Map<Object, List<T>> groupBy(String columnName) {
        String res = whereCore()  + orderBy;
    ...
    public List<Object> distinctBy(String columnName) {
        String res = whereCore()  + orderBy + limit;
        return iSession.distinctBy(aClass, columnName, res.trim(), objects);
    }

    public List<?> select(String columnName) {
        String res = whereCore()  + orderBy + limit;
        return iSession.getListSelect(aClass, columnName, res.trim(), objects);
    }

    @Override
    public List<?> selectExpression(String expression) {
        String res = whereCore()  + orderBy + limit;
        return iSession.selectExpression(aClass,expression,res.trim(), objects);
    }
    ...
    @Override
    public void iterator(ITask<T> task) {
        String res;
        if (!this.rawSelect.trim().isEmpty()) {
            res = this.rawSelect + " WHERE " + whereCore()  + orderBy + limit;
            iSession.iteratorFree(this.aClass, res.trim(), task, this.objects);
        } else {
            res = whereCore()+ orderBy + limit;
            iSession.iterator(this.aClass, task, res.trim(), this.objects);
        }
    }
    ...
    @Override
    public CompletableFuture<List<?>> selectAsync(String columnName) {
    ...
    @Override
    public CompletableFuture<List<?>> selectExpressionAsync(String expression) {
        return CompletableFuture.supplyAsync(() -> selectExpression(expression));
    }

    @Override
    public CompletableFuture<Map<Object, List<T>>> groupByAsync(String columnName) {
        return CompletableFuture.supplyAsync(() -> groupBy(columnName));
    }

    @Override
    public CompletableFuture<List<Object>> distinctByAsync(String columnName) {
        return CompletableFuture.supplyAsync(() -> distinctBy(columnName));
    }

    @Override
    public CompletableFuture<Object> countAsync() {
    ...
    @Override
    public CompletableFuture<Void> dropTableIfExistsAsync() {
        return CompletableFuture.runAsync(this::dropTableIfExists);
    }
    ...
    @Override
    public CompletableFuture<Void> iteratorAsync(ITask<T> task) {
        return CompletableFuture.runAsync(() -> {
            iterator(task);
        });
    }
    ...
    public T getByPrimaryKey(Object primaryKey) {
        return iSession.getByPrimaryKey(aClass, primaryKey);
    }

    @Override
    public CompletableFuture<T> getByPrimaryKeyAsync(Object primaryKey) {
        return CompletableFuture.supplyAsync(() -> getByPrimaryKey(primaryKey));
    }
